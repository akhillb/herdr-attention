...
// Triggered by the `workspace.created` event hook: docks the Next Meeting pane
// on the right of every newly created workspace.
const { execFile } = require('node:child_process');

const bin = process.env.HERDR_BIN_PATH || 'herdr';
const pluginId = process.env.HERDR_PLUGIN_ID || 'cal';
...
if (!targetPane) {
  process.stderr.write('cal: no target pane in workspace.created context\n');
  process.exit(0);
}
...
if (process.env.CAL_DEMO === '1') args.push('--env', 'CAL_DEMO=1');

execFile(bin, args, (err) => {
  if (err) { process.stderr.write(`cal: failed to open pane: ${err.message}\n`); return; }
